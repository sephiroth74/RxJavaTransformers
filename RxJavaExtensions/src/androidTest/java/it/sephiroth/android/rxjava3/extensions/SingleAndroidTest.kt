...
class SingleAndroidTest {
    ...
    fun test006() {
        ...
        var disposable = Single.just(1).debug("s1").debugWithThread("s1-thread")
            .doOnSuccess { latch.countDown() }.autoSubscribe()
        latch.await()
        Assert.assertTrue(disposable.isDisposed)

        latch = CountDownLatch(1)
        disposable = Single.error<Int>(RuntimeException()).debug("s2").debugWithThread("s2-thread")
            .doOnError { latch.countDown() }.autoSubscribe()
        latch.await()
        Assert.assertTrue(disposable.isDisposed)
    ...
    @Test
    fun test007() {
        Single.just(listOf(1, 2, 3, 4, 5)).mapList { it * 2 }.test().await().assertComplete()
            .assertValues(listOf(2, 4, 6, 8, 10))
    }

    @Test
    fun test008() {
        Single.just(listOf(1, 2, 3, 4, 5)).firstInList().test().await().assertComplete()
            .assertValue(1)
    ...
    @Test
    fun test010() {
        Single.just(listOf(1, 2, 3, 4, 5)).asObservable().test().await().assertComplete()
            .assertValues(1, 2, 3, 4, 5)
        Single.just(emptyList<Int>()).asObservable().test().await().assertComplete()
            .assertNoValues()
    }

    @Test
    fun test011() {
        Single.just(listOf(1, 2, 3, 4, 5)).firstInList { it % 2 == 0 }.test().await()
            .assertComplete().assertValue(2)
        Single.just(emptyList<Int>()).firstInList { it % 2 == 0 }.test().await().assertComplete()
            .assertNoValues()
    }
